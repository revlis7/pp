...
Ext.onReady(function () {
	...
	var storeBaseUrl = '/ts/index.php/csr_corp/view';
	var mode = 0;
	var recentMode = 'null';
	var catMode = 'all';
	var FSCChannel = 'null';
	...
	var csrCorpStore = Ext.create('Ext.data.JsonStore', {
		fields: [{
			...
			name: 'csr_corp_contact4_name'
		}, {
			name: 'csr_corp_contact4_title'
		}, {
			name: 'csr_corp_contact4_phone'
		}, {
			name: 'csr_corp_contact4_email'
		}, {
			name: 'csr_corp_contact4_CV'
		}, {
			name: 'csr_corp_contact5_name'
		}, {
			name: 'csr_corp_contact5_title'
		}, {
			name: 'csr_corp_contact5_phone'
		}, {
			name: 'csr_corp_contact5_email'
		}, {
			name: 'csr_corp_contact5_CV'
		}, {
	...
	var csrCorpListStore = Ext.create('Ext.data.JsonStore', {
        ...
        listeners: {
            totalcountchange: function(){
                corp_list.down('component#status').update({count: csrCorpListStore.getTotalCount()});
			}
		}
	});
	var csrCorpFollowStore = Ext.create('Ext.data.JsonStore', {
		fields: [{
			name: 'csr_corp_id',
			type: 'integer'
		}, {
			...
			name: 'csr_corp_follow_result',
			type: 'string'
		}],
		proxy: {
			type: 'ajax',
			url: '/ts/index.php/csr_corp/follow_view',
			reader: {
				type: 'json',
				root: 'data'
			}
	...
	var csrCorpFollowsStore = Ext.create('Ext.data.JsonStore', {
		pageSize: 100,
		remoteSort: true,
		autoLoad: true,
		leadingBufferZone: 100,
		...
		proxy: {
			type: 'ajax',
			url: '/ts/index.php/csr_corp/follows_view',
			reader: {
				type: 'json',
				root: 'data',
				totalProperty: 'totalCount'
	...
	var corp_list = Ext.create('Ext.grid.Panel', {
		id: 'corp_list',
			...
			items: [{
				...
				changeHandler: function (cycleBtn, activeItem) {
					...
					csrCorpListStore.setProxy({
						type: 'ajax',
						url: '/ts/index.php/csr_corp/view?mode=' + mode + '&recentMode=' + recentMode + '&catMode=' + catMode + '&FSCChannel=' + encodeURI(FSCChannel) + '&PStart=' + PStart + '&PEnd=' + PEnd + '&lastFollow=' + encodeURI(lastFollow),
						reader: {
							type: 'json',
							root: 'data',
							totalProperty: 'totalCount'
		...
		columns: [{
			...
			items: [{
				...
				handler: function (grid, rowIndex, colIndex) {
					...
					csrCorpStore.setProxy({
						type: 'ajax',
						url: '/ts/index.php/csr_corp/get?csr_corp_id=' + temp_csr_corp_id,
						reader: {
							type: 'json',
							root: 'data'
						}
					...
					csrCorpStore.load(function (records, operation, success) {
						infoForm.getForm().loadRecord(records[0]);
					});
					followpanel.down('form').down('hiddenfield[name="csr_corp_id"]').setValue(temp_csr_corp_id);
		...
		},*/
{
			...
			dataIndex: 'csr_corp_contact1_name',
			text: '联系人1姓名',
			filtable: true,
			style: "text-align:center;",
			align: 'center',
			...
			dataIndex: 'csr_corp_contact1_title',
			text: '联系人1职位',
			filtable: true,
			style: "text-align:center;",
			align: 'center',
			...
			dataIndex: 'csr_corp_contact1_phone',
			text: '联系人1电话',
			filtable: true,
			style: "text-align:center;",
			align: 'center',
			...
			dataIndex: 'csr_corp_contact1_email',
			text: '联系人1邮箱',
			filtable: true,
			style: "text-align:center;",
			align: 'center',
			...
			dataIndex: 'csr_corp_FSC_channel',
			text: '渠道部负责人',
			filtable: true,
			style: "text-align:center;",
			align: 'center',
			...
			dataIndex: 'csr_corp_follow_creator',
			text: '最后跟进人',
			filtable: true,
			style: "text-align:center;",
			align: 'center',
			...
			dataIndex: 'csr_corp_follow_update_ts',
			text: '最后跟进时间',
			filtable: true,
			style: "text-align:center;",
			align: 'center',
	...
	var infoForm = Ext.create('Ext.form.Panel', {
		...
		items: [{
			...
			items: [{
				xtype: 'textareafield',
				name: 'csr_corp_director',
				fieldLabel: '公司主要股东<br />及其主要履历'
			}, {
				xtype: 'textareafield',
				name: 'csr_corp_legalperson',
				fieldLabel: '公司法人<br />及主要履历'
			}, {
				xtype: 'textareafield',
				name: 'csr_corp_CEO',
				fieldLabel: '公司总经理<br />及主要履历'
			}, {
				xtype: 'textareafield',
				name: 'csr_corp_manager',
				fieldLabel: '公司主要高管<br />及主要履历'
			}, {
				xtype: 'fieldset',
				title: '====<b>联系人1</b>====',
				border: 0,
			...
			items: [{
				xtype: 'textfield',
				name: 'csr_corp_yearly_amount',
				fieldLabel: '公司最近<br />年度募资额'
			}, {
				xtype: 'numberfield',
				name: 'csr_corp_FP_no',
				fieldLabel: '公司理财师<br />人数'
			}, {
				xtype: 'numberfield',
				name: 'csr_corp_branch_no',
				fieldLabel: '公司分公司<br />个数'
			}, {
				xtype: 'combo',
				queryMode: 'local',
				store: chCorpBizStyleList,
				valueField: 'id',
				displayField: 'text',
				forceSelection: true,
				name: 'csr_corp_biz_style',
				...
				xtype: 'textfield',
				name: 'csr_corp_biz_dir_percent',
				fieldLabel: '公司直销<br />占销售额比例'
			}, {
				xtype: 'textfield',
				name: 'csr_corp_biz_type',
				fieldLabel: '公司主销<br />产品类别'
			}, {
				xtype: 'textfield',
				name: 'csr_corp_biz_partner',
				fieldLabel: '公司主要金融<br />机构合作伙伴'
			}, {
				xtype: 'textareafield',
				name: 'csr_corp_biz_plusinfo',
				fieldLabel: '公司理财业务<br />其他情况'
			}]
		}, {
			xtype: 'fieldset',
			title: '====<b>投行团队</b>====',
			...
			items: [{
				xtype: 'textfield',
				name: 'csr_corp_yearly_issue',
				fieldLabel: '公司最近年度<br />发行额'
			}, {
				xtype: 'numberfield',
				name: 'csr_corp_IP_no',
				fieldLabel: '公司投行团队<br />人数'
			}, {
				xtype: 'combo',
				queryMode: 'local',
				store: chCorpInvStyleList,
				valueField: 'id',
				displayField: 'text',
				forceSelection: true,
				name: 'csr_corp_inv_style',
				fieldLabel: '公司投行发行<br />风格'
			}, {
				xtype: 'combo',
				queryMode: 'local',
				store: chCorpInvChtypeList,
				valueField: 'id',
				displayField: 'text',
				forceSelection: true,
				name: 'csr_corp_inv_chtype',
				fieldLabel: '公司投行发行<br />渠道'
			}, {
				xtype: 'textareafield',
				name: 'csr_corp_inv_case',
				fieldLabel: '公司投行业务<br />成功案例'
			}, {
				xtype: 'textareafield',
				name: 'csr_corp_inv_plusinfo',
				fieldLabel: '公司投行业务<br />其他情况'
			}]
		}, {
			xtype: 'fieldset',
			title: '====<b>合作信息</b>====',
			...
			items: [{
				xtype: 'textfield',
				name: 'csr_corp_coop_partner',
				fieldLabel: '目前主要合作<br />竞争方'
			}, {
				xtype: 'textfield',
				name: 'csr_corp_coop_FSC_dir',
				fieldLabel: '与我司<br />合作方向'
			}, {
				xtype: 'textfield',
				name: 'csr_corp_coop_FSC_wish',
				fieldLabel: '与我司<br />合作意愿'
			}, {
				xtype: 'textfield',
				name: 'csr_corp_follow',
				fieldLabel: '后续跟进策略'
				...
				xtype: 'textfield',
				name: 'csr_corp_FSC_channel',
				fieldLabel: '我司渠道部<br />负责人'
			}, {
				xtype: 'textfield',
				name: 'csr_corp_FSC_pdt',
				fieldLabel: '我司产品部<br />负责人'
			}]
		}]
	});
		...
		items: [{
			...
			dockedItems: [{
				...
				items: [{
					...
					handler: function () {
						this.up('form').getForm().submit({
							url: '/ts/index.php/csr_corp/follow_update',
							submitEmptyText: false,
							waitMsg: 'Saving Data...',
							success: function (form, action) {
								...
								followpanel.down('form').down('hiddenfield[name="csr_corp_id"]').setValue(action.result.csr_corp_id);
								//csrCorpStore.load();
							}
							//,
							//failure: function(form, action) {
			...
			columns: [{
				...
				text: '跟进结果',
				dataIndex: 'csr_corp_follow_result',
				filtable: true,
				style: "text-align:center;",
				align: 'left',
				width: 120
				...
				text: '跟进状况',
				dataIndex: 'csr_corp_FSC_follow_status',
				filtable: true,
				style: "text-align:center;",
				align: 'left',
				width: 350,
				...
				text: '机会和问题',
				dataIndex: 'csr_corp_FSC_opp_and_prob',
				filtable: true,
				style: "text-align:center;",
				align: 'left',
				width: 250,
				...
				text: '解决方案',
				dataIndex: 'csr_corp_FSC_solution',
				filtable: true,
				style: "text-align:center;",
				align: 'left',
				width: 200,
	...
	var csrCorpEditPanel = Ext.create('Ext.panel.Panel', {
		autoScroll: true,
		region: 'center',
		layout: 'card',
		items: [
		...
		items: [{
			...
			items: [{
				...
				items: [{
					...
					width: 500,
					name: 'csr_corp_name',
					fieldLabel: '<b>公司名称(必填)</b>',
					allowBlank: false,
					blankText: '该项必须填写'
				}, {
					xtype: 'textfield',
					name: 'csr_corp_bname',
					fieldLabel: '<b>公司简称(必填)</b>',
					allowBlank: false,
					blankText: '该项必须填写'
				}, {
					xtype: 'textfield',
				...
				items: [{
					...
					width: 700,
					name: 'csr_corp_director',
					fieldLabel: '公司主要股东及其主要履历'
				}, {
					xtype: 'textareafield',
					width: 700,
					name: 'csr_corp_legalperson',
					fieldLabel: '公司法人及主要履历'
				}, {
					xtype: 'textareafield',
					width: 700,
					name: 'csr_corp_CEO',
					fieldLabel: '公司总经理及主要履历'
				}, {
					xtype: 'textareafield',
					width: 700,
					name: 'csr_corp_manager',
					fieldLabel: '公司主要高管及主要履历'
				}, {
					xtype: 'fieldset',
					title: '====<b>联系人1</b>====',
					border: 0,
				...
				items: [{
					...
					xtype: 'textfield',
					name: 'csr_corp_biz_partner',
					fieldLabel: '公司主要金融机构合作伙伴'
				}, {
					xtype: 'textareafield',
					width: 700,
					name: 'csr_corp_biz_plusinfo',
					fieldLabel: '公司理财业务其他情况'
				}]
			}, {
				xtype: 'fieldset',
				title: '====<b>投行团队</b>====',
				...
				items: [{
					...
					width: 700,
					name: 'csr_corp_inv_case',
					fieldLabel: '公司投行业务的成功案例'
				}, {
					xtype: 'textareafield',
					width: 700,
					name: 'csr_corp_inv_plusinfo',
					fieldLabel: '公司投行业务的其他情况'
				}]
			}, {
				xtype: 'fieldset',
				title: '====<b>合作信息</b>====',
	...
	var topToolbar = Ext.create('Ext.toolbar.Toolbar', {
		...
		items: [{
			...
			xtype: 'tbtext',
			id: 'headerTitle',
			text: '<span class="app-header2">经纪机构客户列表</span>'
		}, {
			xtype: 'box',
			flex: 1
		}, {
			...
			handler: function () {
				Ext.getCmp('infoPanel').up('panel').getLayout().setActiveItem(0);
				Ext.getCmp('headerTitle').setText('<span class="app-header2">经纪机构客户列表</span>');
				Ext.getCmp('BtnFollowsView').hide();
				Ext.getCmp('BtnCorpView').show();
			}
		}]
	});
	Ext.getCmp('viewport').add(csrCorpEditPanel);
	...
	var reportersStore = Ext.create('Ext.data.JsonStore', {
		...
		proxy: {
			type: 'ajax',
			url: '/ts/index.php/user/view_reporters?access_mode=csr_corp/save',
			reader: {
				type: 'json',
				root: 'data'
			}
	...
	var csrCorpFollowWin = Ext.create('Ext.window.Window', {
		resizeable: false,
		closeAction: "hide",
		closable: true,
		title: '经济机构客户跟踪',
		...
		items: [{
			...
			listeners: {
				celldblclick: function (grid, td, cellIndex, record, tr, rowIndex) {
					//sampleStore.removeAt(rowIndex);
					var temp_csr_corp_id = grid.getStore().getAt(rowIndex).get("csr_corp_id");
					window.open('/ts/index.php/csr_corp?csr_corp_id=' + temp_csr_corp_id);
				}
			},
			columns: [
			/*{
		...
		},*/
{
			...
			series: [{
				...
				listeners: {
					'itemclick': function (a, b, c) {
						...
						csrCorpFollowsStore.setProxy({
							type: 'ajax',
							url: '/ts/index.php/csr_corp/follows_view?s_username=' + encodeURI(a.yField) + '&s_day=' + ((a.value[0].getYear() < 1900) ? (1900 + a.value[0].getYear()) : a.value[0].getYear()) + "-" + (a.value[0].getMonth() + 1) + "-" + a.value[0].getDate() + '&s_mode=day',
							reader: {
								type: 'json',
								root: 'data',
								totalProperty: 'totalCount'
		...
		attendingReportersStore.load(function (at_records, at_operation, at_success) {
			Ext.Array.forEach(records, function (record) {
				if (record.get("branch") == "南京财富中心") {
					...
					personStatDailyFields.push({
						name: unescape(record.get('realname')),
						type: 'integer'
					});
				}
			});
			...
			personStatDailyStore = Ext.create('Ext.data.JsonStore', {
				...
				proxy: {
					type: 'ajax',
					url: '/ts/index.php/csr_corp/get_daily_stats',
					reader: {
						type: 'json',
						root: 'data'
					}
	...
	Ext.getCmp('corp_list').on({
		hide: slidepanelleft
	});
	//csrCorpStore.load();
	//csrChannelStore.load();
	//csrCorpStore.loadPage(1);
	//listControl.load(function(records, operation, success) {
	if (params.csr_corp_id > 0) {
		Ext.getCmp('infoPanel').up('panel').getLayout().setActiveItem(1);
		var temp_csr_corp_id = params.csr_corp_id;
		csrCorpStore.setProxy({
			type: 'ajax',
			url: '/ts/index.php/csr_corp/get?csr_corp_id=' + temp_csr_corp_id,
			reader: {
