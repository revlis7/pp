  ...
  function slideproj(co){
    ...
    var topToolbar=Ext.create('Ext.toolbar.Toolbar',{
			...
			items:[
			{
				...
				itemId:"ListBtn",
				handler:function(){
                    Ext.getCmp('headerTitle').setText('<span class="app-header2">浮动收益类：在售产品列表</span>');
				...
				handler:function(){
                    Ext.getCmp('headerTitle').setText('<span class="app-header2">浮动收益类：近期结束产品列表</span>');
                    //Ext.getCmp("endProjListBtn").hide();
                    //Ext.getCmp("ListBtn").show();
					projAllStore.setProxy({
						type: 'ajax',
						url: '/ts/index.php/proj/view?c=2&r=true&e=1',
						reader:	{
							type: 'json',
							root: 'data'
						}
		...
		text:'产品信息',columns:[
		{
			...
			items: [{
				...
				handler: function(grid, rowIndex, colIndex) {
                    ...
                    var proj_id=grid.getStore().getAt(rowIndex).get("id");
